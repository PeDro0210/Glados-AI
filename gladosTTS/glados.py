import torch
import numpy as np
from gladosTTS.utils.tools import prepare_text
...
class tts_runner:
    def __init__(self, use_p1: bool=False, log: bool=False):
        self.log = log
        if use_p1:
            self.emb = torch.load('src\gladosTTS\models\emb\glados_p1.pt')
        else:
            self.emb = torch.load('src\gladosTTS\models\emb\glados_p2.pt')
        # Select the device
        ...
        self.vocoder = torch.jit.load('src\\gladosTTS\\models\\vocoder-gpu.pt', map_location=self.device)
        for i in range(2):
